...
function copyFilesBatch($baseDir, $targetDir, $fileTypes) {
    ...
    foreach ($fileTypes as $fileType) {
        ...
        foreach (glob($sourceDir . '/*') as $file) {
            ...
            foreach (glob("$sourceDir/index.*.$fileType") as $file2) {
    ...
    $filesJson = json_decode(file_get_contents(__DIR__.'/../asset/asset-all.json'), true);
    ...
    file_put_contents(__DIR__.'/../asset/asset-manifest.json', json_encode(['file' => $mainFiles], JSON_PRETTY_PRINT));
    file_put_contents(__DIR__.'/../asset/asset-all.json', json_encode($copiedFiles, JSON_PRETTY_PRINT));
}

// Указываем базовую директорию исходных файлов, целевую директорию и расширения файлов

$baseDirectory = __DIR__.'/../react/build/static';//откуда извлекать файлы
$targetDirectory = __DIR__.'/../static/';//куда сувать файлы
